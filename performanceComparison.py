...
def re_scraper(url):
    ...
    titles = re.findall('<div class="cl f14">.*?<a class="mr5".*?>(.*?)</a>', res.text, re.S)
    authors = re.findall('<div class="cl lh18 bbs_author">.*?<a.*?>(.*?)</a>', res.text, re.S)
...
def bs_scraper(url):
    ...
    titles = soup.select('div.cl.f14 > a')
    authors = soup.select('div.cl.lh18.bbs_author > cite > a')
    for title, author in zip(titles, authors):
        info = {
